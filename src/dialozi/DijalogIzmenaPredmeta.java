			...
			public void actionPerformed(ActionEvent e) {
				...
				if (onemoguciTxtField == 2) {
					...
					if (semestar != godinaIzvodjenja * 2 && semestar != godinaIzvodjenja * 2 - 1) {
						JOptionPane.showMessageDialog(null,"Za " + godinaIzvodjenja + ". godinu " + " morate izabrati ili " + (godinaIzvodjenja * 2 - 1)+ " ili " + godinaIzvodjenja * 2 + " semestar, ili promenite godinu");
						onemoguciTxtField = 0;
						return;
					}

					String profesor = BazaPredmeta.getInstance().getPredmeti().get(selektovanaVrsta).getPredmetniProfesor();
