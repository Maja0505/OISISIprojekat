...
public class DijalogDodajPredmet {
	public DijalogDodajPredmet(boolean vidljiv) {
		JDialog  dodajPredmeta = new ModalniDijalog(new JFrame(), "Dodaj predmet", true,600,750);
		
		
		JLabel statusBar = new JLabel();
		dodajPredmeta.add(statusBar,BorderLayout.SOUTH);
		statusBar.setPreferredSize(new Dimension(dodajPredmeta.getWidth(), 25));
		
		JPanel panel = new JPanel();
		panel.setLayout(new GridBagLayout());
		panel.setBackground(Color.WHITE);
		dodajPredmeta.add(panel,BorderLayout.CENTER);
		
		dodajPredmeta.add(panel);
		...
		dodajPredmeta.setVisible(vidljiv);
	}
}
